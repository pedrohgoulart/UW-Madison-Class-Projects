...
public class MapApp {
	...
	public static NavigationGraph createNavigationGraphFromMapFile(String graphFilepath)
		...
		String[] propertynames = new String[names.length-2];
		for (int i = 0; i < names.length - 2; i++) {
			propertynames[i] = names[i + 2];
		}
		List<Location> locations = new ArrayList<Location>();
		NavigationGraph navigraph = new NavigationGraph(propertynames);
		while (scanFile.hasNextLine()) {
			String[] temp = scanFile.nextLine().split(" ");
			if (temp.length != names.length)
				throw new InvalidFileException("invalid file");
			Location src = new Location(temp[0].toLowerCase());
			Location des = new Location(temp[1].toLowerCase());
			navigraph.addVertex(src);
			locations.add(des);
			List<Double> property = new ArrayList<Double>();
			try {
				for (int i = 2; i < temp.length; i++) {
					property.add(Double.parseDouble(temp[i]));
				}
			} catch (Exception e) {
				throw new InvalidFileException("invalid file");
			}
			Path edge = new Path(src, des, property);
			navigraph.addEdge(src, des, edge);
		}
		scanFile.close();
		for(Location locs : locations){
			if(navigraph.getLocationByName(locs.getName()) == null){
				navigraph.addVertex(locs);
			}
		}
		return navigraph;
	}
